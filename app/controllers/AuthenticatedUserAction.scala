...
/**
  ...
  */
class AuthenticatedUserAction @Inject()(parser: BodyParsers.Default)(implicit val ec: ExecutionContext)
  extends ActionBuilderImpl(parser) {
    ...
    override def invokeBlock[A](request: Request[A], block: Request[A] => Future[Result]): Future[Result] = {
        CSRF.getToken(request).fold {
            ...
            request.session.get(models.Global.SESSION_USERNAME_KEY).fold {
                Future.successful(Redirect(routes.HomeController.index()))
            } { username =>
                block(request)
            }
...
class Authenticated1UserAction @Inject() (parser: BodyParsers.Default)(implicit ec: ExecutionContext)
  extends ActionBuilderImpl(parser) {
